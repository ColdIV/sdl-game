...
void GameView::centerPlayer (bool value) {
    this->playerCentered = value;
}

bool GameView::isPlayerCentered () {
    return this->playerCentered;
}

void GameView::move (SDL_Point position, int32_t width, int32_t height) {
    if (this->isPlayerCentered()) {
        this->position.x = position.x + width / 2 - this->getWidth() / 2;
        this->position.y = position.y + height / 2 - this->getHeight() / 2;
    } else {
